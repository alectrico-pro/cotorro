...
(defmodule MAIN (export ?ALL))
  ...
  (defglobal ?*archivo_id* = 0 )

;abrir archivo
 (defrule MAIN::initial-facts
   (declare (salience 1000))
  =>
   (close ?*archivo_id*)
   (open "interface_alectico_site.facts" ?*archivo_id* "w")
 )
...
;;************
 ...
 ?f1<-(initial-fact)
  =>   
   (printout t crlf )
   (printout t "-----ALEX: Aléctrico Expert. Análisis Inteligente de Riesgos Eléctricos-------------" crlf)
...
(defrule IMPRIMIR::sair
 ...
 (defrule IMPRIMIR::imprimir-inferencias ""
  ...
  (not (sair))
   ...
   (format ?*archivo_id* "{  %-24s %-24d%  2d%% %-10s %n" ?onome ?ovalor ?per ?rest)


    else
   (format t "  %-24s %-24s%  2d%% %-10s %n" ?onome ?ovalor ?per ?rest)
   (format ?*archivo_id* "{inferencia: {{nombre: %s } {valor: %s } {certeza: %2d%%} {porque: %s}} %n" ?onome ?ovalor ?per ?rest)  
   )
   )
