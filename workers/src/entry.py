...
async def anotar_tokens_pagados_promocionales( env, buy_order, fono, cantidad ):
    ...
    for orden in range(1, cantidad + 1 ):
      pedido = { 'token': {'orden': orden, 'expira_en': str(fecha_en_el_vencimiento), 'buy_order': buy_order, 'fono': fix_fono( fono), "amount": 0, "acuñado_en": json.dumps( date.today().isoformat()) }}
